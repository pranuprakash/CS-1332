...
public class ArrayStack<T> {
    ...
    public void push(T data) {
        if (data == null) {
            throw new IllegalArgumentException("Entered argument is invalid. Cannot enter null data");
        }

        if ((size + 1) > backingArray.length) { // size >= backingArray.length
    ...
    public T pop() {
        if (size == 0) {
            throw new NoSuchElementException("Stack is empty. Cannot remove from an empty stack.");
        }

        T returnData = backingArray[--size]; // backingArray[size - 1]
